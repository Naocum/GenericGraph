...
TSharedRef<FMissionGraphDragConnection> FMissionGraphDragConnection::New(const TSharedRef<SGraphPanel>& GraphPanel, const FDraggedPinTable& DraggedPins)
{
	TSharedRef<FMissionGraphDragConnection> Operation = MakeShareable(new FMissionGraphDragConnection(GraphPanel, DraggedPins));
	Operation->Construct();

	return Operation;
}

void FMissionGraphDragConnection::OnDrop(bool bDropWasHandled, const FPointerEvent& MouseEvent)
{
...
void FMissionGraphDragConnection::OnDragged(const class FDragDropEvent& DragDropEvent)
{
...
void FMissionGraphDragConnection::HoverTargetChanged()
{
	...
	else if (UEdNode_MissionGraphNode* TargetNodeObj = Cast<UEdNode_MissionGraphNode>(GetHoveredNode()))
	{
		TArray<UEdGraphPin*> ValidSourcePins;
		ValidateGraphPinList(/*out*/ ValidSourcePins);
...
#define LOCTEXT_NAMESPACE "AssetSchema_MissionGraph"
				Response = FPinConnectionResponse(CONNECT_RESPONSE_DISALLOW, LOCTEXT("PinError", "Not a valid UMissionGraphEdNode"));
...
FMissionGraphDragConnection::FMissionGraphDragConnection(const TSharedRef<SGraphPanel>& GraphPanelIn, const FDraggedPinTable& DraggedPinsIn)
...
FReply FMissionGraphDragConnection::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
	TArray<UEdGraphPin*> ValidSourcePins;
	ValidateGraphPinList(/*out*/ ValidSourcePins);
...
FReply FMissionGraphDragConnection::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FMissionGraphDragConnection::DroppedOnPanel(const TSharedRef< SWidget >& Panel, FVector2D ScreenPosition, FVector2D GraphPosition, UEdGraph& Graph)
{
	// Gather any source drag pins
...
void FMissionGraphDragConnection::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
